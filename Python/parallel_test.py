...
# Set up processors
# path is ~/.python/pofile_MACHINE_NAME/security/ipcontroller-client.json
...
for i in range(len(Runs)):
    ...
    dv.execute('A_local=g.interp_to_g1(VT,T)',block=True)
    VT_l = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(WT,T)',block=True)
    WT_l = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(V,T)',block=True)
    V_l = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(W,T)',block=True)
    W_l = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(W,T)',block=True)
    th = dv.gather('A_local').get()[1]
    dv.execute('A_local=g.interp_to_g1(T,T)',block=True)
    T_l  = dv.gather('A_local').get()[0]

    dv['Ty'] = c.ddy_cgrid_centered(c.mnc('Tav.nc', 'THETA'))
    dv['Tz'] = c.ddz_cgrid_centered(c.mnc('Tav.nc', 'THETA'))
    dv.execute('A_local=g.interp_to_g1(Ty,T)',block=True)
    Ty_l  = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(Tz,T)',block=True)
    Tz_l  = dv.gather('A_local').get()[0]

    # ADD in netcdf write #########
    f = netCDF4.Dataset(str(fname)+'Remapped.nc','w')
    ...
    dv.execute('A_local=g.interp_to_g1(VpTp,T)',block=True)
    VpTp_l = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(WpTp,T)',block=True)
    WpTp_l = dv.gather('A_local').get()[0]
    f = netCDF4.Dataset(str(fname)+'Eddy_L.nc','w')
    f.createDimension('T',g.ng)
    f.createDimension('Y',c.Ny)
    f.createDimension('X',c.Nx)
    ...
    dv.execute('A_local=g.interp_to_g1(WTbar,T)',block=True)
    WbarTbar_l = dv.gather('A_local').get()[0]
    dv.execute('A_local=g.interp_to_g1(VTbar,T)',block=True)
    VbarTbar_l = dv.gather('A_local').get()[0]
    
    f = netCDF4.Dataset(str(fname)+'Remapped_bar.nc','w')
    f.createDimension('T',g.ng)
    f.createDimension('Y',c.Ny)
    f.createDimension('X',c.Nx)
